...
class CardRenderer extends Component {
    ...
    async getStateFromProps(prevProps) {
        ...
        if (
            prevProps.attack1.attack1Type !== attack1Type ||
            prevProps.attack1.attack1Amount !== attack1Amount
        ) {
            nextState.attack1Img =
                attack1Type !== '' && attack1Amount !== ''
                    ? await this.getDynamicImg(
                          `${attack1Amount}-${attack1Type}.png`
                      )
                    : null
        }
        if (
            prevProps.attack2.attack2Type !== attack2Type ||
            prevProps.attack2.attack2Amount !== attack2Amount
        ) {
            nextState.attack2Img =
                attack2Type !== '' && attack2Amount !== ''
                    ? await this.getDynamicImg(
                          `${attack2Amount}-${attack2Type}.png`
                      )
                    : null
        }
    ...
    updateImgPos = (event) => {
        ...
        this.setState({
            [`${attrs.name}X`]: attrs.x,
            [`${attrs.name}Y`]: attrs.y,
        })
    }
    ...
    render() {
        ...
        const {
            ...
            mainPictureY,
            updateImgPos,
            attack1: { attack1Name, attack1Dammage, attack1Info },
            attack2: { attack2Name, attack2Dammage, attack2Info },
        } = this.props
                ...
                <Layer>
                    ...
                    <Group x={29} y={627} width={570}>
                        ...
                        {retreatVisible && retreatImg && (
                            <KonvaImage x={355} y={16} image={retreatImg} />
                        )}
                    </Group>
                    {description !== '' && (
